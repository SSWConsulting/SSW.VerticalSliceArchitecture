...
- **Slices**: Each use case is a folder under `Features/{FeatureName}/` (e.g. `CreateHero/`) with one class per file. The endpoint inherits from `Endpoint<TRequest, TResponse>` or `EndpointWithoutRequest<TResponse>`, and the namespace matches the folder, e.g. `Features.Heroes.CreateHero`
...
#### Endpoint Structure
Each slice folder under `Features/{FeatureName}/` contains, with one class per file:
